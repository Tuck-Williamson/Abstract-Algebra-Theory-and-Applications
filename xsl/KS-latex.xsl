...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:text>bwminimalstyle, fonttitle=\normalfont\scshape, attach title to upper, after
    title={\space}, after upper={\space\space\hspace*{\stretch{1}}\(\blacksquare\)},&#xa;</xsl:text>
</xsl:template> -->
    <xsl:template match="exercises" mode="tcb-style">
        <xsl:text> before title={\newpage}, &#xa;</xsl:text>
    </xsl:template>
    <xsl:template match="divisionexercise" mode="tcb-style">
        <xsl:text>after skip=20cm, &#xa;</xsl:text>
        <!-- \tcbset{ divisionexercisestyle/.style={bwminimalstyle, runintitlestyle,
        exercisespacingstyle, left=5ex, breakable, before upper app={\setparstyle} } } -->
    </xsl:template>
    ...
    <xsl:text>\tcbset{ aatahistoricalstyle/.style={size=minimal, boxrule=-0.3pt, frame empty,
    colback=white, colbacktitle=white, coltitle=black, fonttitle=\itshape\bfseries, before skip=3ex,
    bottomtitle=2ex, breakable, title={\raisebox{3pt}{\framebox[0.3\textwidth]{\relax}} \hfill
    {Historical Note} \hfill \raisebox{3pt}{\framebox[0.3\textwidth]{\relax}}}} }&#xa;</xsl:text>
